...
initJsBridge(() => {
    ...
    if (isiOS) {
        ...
        mapCoordinate = userInfo.MapCoordinate
        mapCoordinate = userInfo.MapCoordinate
    }
    if (isAndroid) {
        var userInfo = JSON.parse(window.localStorage.getItem('userInfo'))
    ...
    new Vue({
        ...
        methods: {
            ...
            addCart(index, type) {
                if (index >= 0 && !type) {
                    ...
                    var food = JSON.stringify(Object.assign({}, this.detailInfo))
            ...
            decreas(index, type) {
                if (index >= 0 && !type) {
                    let index1 = this.cartFoods.indexOf(this.list[index])
        ...
        mounted() {
            this.projectListQuery.mapCoordinate = mapCoordinate
            this.queryParam.mapCoordinate = mapCoordinate
            this.$indicator.open({
                text: '加载中...',
                spinnerType: 'fading-circle'
            });
