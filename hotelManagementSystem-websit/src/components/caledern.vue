<template>
  <div class="datepicker">
    ...
    <div class="datepicker-wrapper" ref="popup" v-transfer="transfer" v-show="isWrapperShow" style="width:800px" >
        ...
        <template v-for="(p, pan) in pane" >
          <div class="datepicker-inner">
            <div class="datepicker-body">
              <p style="font-size:14px; color:#000"> <Icon type="md-arrow-dropleft-circle" @click="preNextMonthClick(0)" v-if="pan==0" style="margin-right:20px;font-size:18px"/>{{stringifyDayHeader(currDate, pan)}}<Icon type="md-arrow-dropright-circle" @click="preNextMonthClick(1)" style="margin-left:20px;font-size:18px" v-if="pan==1"/></p>
              <div class="datepicker-weekRange">
                <span v-for="(w,index) in text.daysOfWeek" :key="index" style="width:45px;height:40px;margin:0">{{w}}</span>
              </div>
              <div class="datepicker-dateRange" style="margin-left:15px">
                <span v-for="(d,index2) in dateRange[pan]" class="day-cell" :class="getItemClasses(d)" :data-date="stringify(d.date)" @click="daySelect(d, $event)" :key="index2" style="margin-left:10px;"><div>
            ...
            <div class="datepicker-body">
              ...
              <div class="datepicker-monthRange">
                <template v-for="(m, $index) in text.months">
                  <span :class="{'datepicker-dateRange-item-active':(text.months[parse(value).getMonth()]  === m) &&currDate.getFullYear() + pan === parse(value).getFullYear()}" @click="monthSelect(stringifyYearHeader(currDate, pan), $index)">{{m.substr(0,3)}}</span>
                </template>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    preNextMonthClick (flag) {
      ...
      const date = this.currDate.getDate()
      if (flag === 0) {
        let form1 = `${new Date().getFullYear()}-${new Date().getMonth() + 1}-01`
        let form2 = `${this.currDate.getFullYear()}-${this.currDate.getMonth() + 1}-01`
        let nowDate = new Date(form1).getTime()
        let time = new Date(form2).getTime()
        const preMonth = time > nowDate ? this.getYearMonth(year, month - 1) : this.getYearMonth(year, month)
        this.currDate = new Date(preMonth.year, preMonth.month, date)
        this.changePane(preMonth.year, preMonth.month, this.pane)
      } else {
        let form1 = `${new Date().getFullYear()}-${new Date().getMonth() + 1}-01`
        let form2 = `${this.currDate.getFullYear()}-${this.currDate.getMonth() + 1}-01`
        let nowDate = new Date(form1).getTime() + 30 * 24 * 60 * 60 * 1000
        let time = new Date(form2).getTime()

        const nextMonth = time < nowDate ? this.getYearMonth(year, month + 1) : this.getYearMonth(year, month)
